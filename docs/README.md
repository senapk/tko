# Índice da documentação

Este diretório reúne guias de referência e manutenção do TKO.

## Por perfil

### Contribuidores e mantenedores

- [Guia de contribuição](../CONTRIBUTING.md)
- [Arquitetura técnica](ARCHITECTURE.md)
- [Guia de testes e QA](TESTING.md)

### Professores

- [Guia para criar repositórios de tarefas](../wiki/Criando-Atividades.md)
- [Criando testes e conversões](../wiki/Criando-Tarefas-e-Testes.md)
- [Ciclo de vida de tarefas](TASK_LIFECYCLE.md)
- [Exemplos end-to-end](EXAMPLES.md)
- [Especificação de formatos](FORMATS.md)

### Alunos e uso geral

- [FAQ](FAQ.md)
- [Referência rápida da CLI](REFERENCE.md)

## Ordem sugerida de leitura

1. [FAQ](FAQ.md)
2. [Referência rápida da CLI](REFERENCE.md)
3. [Ciclo de vida de tarefas](TASK_LIFECYCLE.md)
4. [Exemplos end-to-end](EXAMPLES.md)
5. [Arquitetura técnica](ARCHITECTURE.md)

## Mapa dos guias

- [ARCHITECTURE.md](ARCHITECTURE.md): visão técnica de camadas, fluxo e extensão.
- [EXAMPLES.md](EXAMPLES.md): fluxos completos de uso professor/aluno.
- [FAQ.md](FAQ.md): dúvidas e problemas comuns.
- [FORMATS.md](FORMATS.md): formatos de arquivos e convenções principais.
- [LANGUAGE_SUPPORT.md](LANGUAGE_SUPPORT.md): como configurar linguagens.
- [REFERENCE.md](REFERENCE.md): referência rápida de comandos CLI.
- [TASK_LIFECYCLE.md](TASK_LIFECYCLE.md): ciclo de criação/publicação/consumo de tarefas.
- [TESTING.md](TESTING.md): estratégia de validação e QA.

## Governança de documentação

Ao alterar comportamento de uso:

1. Atualize README e guias afetados.
2. Verifique links.
3. Use o checklist de documentação no template de PR.
4. Registre mudanças relevantes no changelog.
