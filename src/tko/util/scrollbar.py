...
class ScrollBar:
    ...
    def count_before_inside_after(self) -> tuple[int, int, int]:
        ...
        block_size: int = int(self.bar_size * visible_ratio)
        block_size = max(self.min_block_size, block_size)
        block_size = min(self.bar_size, block_size)

        position_ratio: float = index / max_index if max_index > 0 else 0.0
        before: int = int((self.bar_size - block_size) * position_ratio)
        after: int = self.bar_size - before - block_size

        return (before, block_size, after)

if __name__ == "__main__":
    sb = ScrollBar(text_size=60, index=0, screen_size=30, bar_size=90)
    print(f"{sb.render()}")
