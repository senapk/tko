...
class TaskTree:
    def __init__(self, settings: Settings, repo: Repository):
        ...
        self.items: list[HasTreeIdentity] = []
    ...
    def get_selected_throw(self) -> HasTreeIdentity:
    ...
    def get_rendered_items(self, items: list[HasTreeIdentity] | None = None) -> list[RText]:
        if items is None:
            items = self.items
    ...
    def expand_all(self):
        for q in self.game.quests.values():
            self.state.expanded.add(q.identity.get_full_key())
