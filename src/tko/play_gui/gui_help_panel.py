...
class GuiHelpPanel:
    ...
    def show(self, frame: Frame) -> None:
        ...
        dx = frame.get_dx() - 2
        help_lines: list[RT] = []
        help_lines.append(RT.parse("[g]<>[]", t(_GuiHelpMsg.SECTION_CONFIG)).center(dx, RT("-")))
        ...
        help_lines.append(RT.parse("[g]<>[]", t(_GuiHelpMsg.SECTION_NAVIGATION)).center(dx, RT("-")))
        ...
        help_lines.append(RT.parse("[g]<>[]", t(_GuiHelpMsg.SECTION_DEFAULT_EDITOR)).center(dx, RT("-")))
