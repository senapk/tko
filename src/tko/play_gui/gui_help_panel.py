...
class GuiHelpPanel:
    ...
    def show(self, frame: Frame) -> None:
        ...
        help_lines.append(RText.format(t(_GuiHelpMsg.SYMBOL_SUGGESTED_OPTIONAL), Symbols.star_filled, Symbols.star_void))
        help_lines.append(RText.format(t(_GuiHelpMsg.SYMBOL_STUDY_WRITE_NOHELP), Symbols.loss_free, Symbols.loss_part, Symbols.loss_zero))
        help_lines.append(RText.format(t(_GuiHelpMsg.SYMBOL_SELF_EVAL),
                                       Symbols.diamond_filled, Symbols.circle_filled, Symbols.task_view,
                                       Symbols.diamond_void, Symbols.circle_void, Symbols.task_view))
        help_lines.append(RText.format("{g}", t(_GuiHelpMsg.SECTION_NAVIGATION)).center(dx, RText("-")))
        help_lines.append(RText.format(t(_GuiHelpMsg.NAV_ARROWS), "↑↓→"))
        help_lines.append(RText.format(t(_GuiHelpMsg.NAV_ENTER), "↲"))
        help_lines.append(RText(t(_GuiHelpMsg.SECTION_INTERFACE), "g").center(dx, RText("-")))
        help_lines.append(RText.format(t(_GuiHelpMsg.INTERFACE_INBOX), GuiKeys.inbox))
        help_lines.append(RText.format(t(_GuiHelpMsg.INTERFACE_ALL), GuiKeys.all_tasks))
        help_lines.append(RText.format(t(_GuiHelpMsg.INTERFACE_PALETTE), GuiKeys.palette))
        help_lines.append(RText.format(t(_GuiHelpMsg.INTERFACE_TIME), GuiKeys.show_duration))
        help_lines.append(RText.format(t(_GuiHelpMsg.INTERFACE_SEARCH), GuiKeys.search))

        help_lines.append(RText(t(_GuiHelpMsg.SECTION_TASKS), "g").center(dx, RText("-")))
        help_lines.append(RText.format(t(_GuiHelpMsg.TASK_DOWNLOAD), GuiKeys.down_task))
        help_lines.append(RText.format(t(_GuiHelpMsg.TASK_DELETE), GuiKeys.delete_folder))
        help_lines.append(RText.format(t(_GuiHelpMsg.TASK_DRAFT), GuiKeys.create_draft))
        help_lines.append(RText.format(t(_GuiHelpMsg.TASK_EVALUATION), GuiKeys.self_evaluate))

        help_lines.append(RText.format("{g}", t(_GuiHelpMsg.SECTION_DEFAULT_EDITOR)).center(dx, RText("-")))
        help_lines.append(RText(t(_GuiHelpMsg.EDITOR_DESC)))
        help_lines.append(RText(t(_GuiHelpMsg.EDITOR_COMMAND), "y").center(dx))
