...
class SolverBuilder:
    ...
    def update_input_function(self, free_run_mode: bool, path_list: list[str]):
        for i in range(len(path_list)):
            ...
            test_input = r'function input(): string { let X: any = input; X.L = X.L || require("fs").readFileSync(0).toString().split(/\r?\n/); return X.L.shift(); } '
            free_input = r'function input(): string { let X: any = input; X.P = X.P || require("readline-sync"); return X.P.question(); }'
            tag = "function input()"
            for line in lines:
                if free_run_mode:
    ...
    def __prepare_ts(self, free_run_mode: bool):
        ...
        for path in self.path_list:
            ...
            new_files.append(os.path.join(copy_dir, os.path.basename(path)))
        self.update_input_function(free_run_mode, new_files)
        transpiler = "npx"
        if os.name == "nt":
            transpiler += ".cmd"
