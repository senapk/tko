...
class DraftsFinderCached:
    ...
    def get_default_dir_for_drafts(self) -> Path:
    ...
    def search_for_solvers(self) -> tuple[bool, Path]:
        if self.__is_source_on(self.task_folder, self.language):
            return True, self.task_folder
        if self.__is_source_on(self.task_folder / self.language, self.language):
            return True, self.task_folder / self.language
        if self.__is_source_on(self.get_default_dir_for_drafts(), self.language):
            return True, self.get_default_dir_for_drafts()
        return False, self.get_default_dir_for_drafts()
    ...
    #         count += 1
    #     if count == 1:
    #         return self.get_default_dir_for_drafts(), 0
    ...
    #             count = int(parts[-1])
    #             if count == 1:
    #                 last_path = self.get_default_dir_for_drafts()
    #             else:
    ...
    def load_source_files(self, extra: list[str] | None = None, cached: bool = False) -> list[Path]:
        ...
        if not self.task_folder.exists():
            return []
        found, drafts = self.search_for_solvers()
        if not found:
            return []
