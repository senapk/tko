...
class Flag:
    ...
    def toggle(self):
        # Fmt.write_text(0, 0, str(self._values))
        # Fmt.getch()
        self._index = (self._index + 1) % 2 #len(self._values)
        return self

    def is_left_bar(self) -> bool:
        return self._bar
    ...
    def get_toggle_sentence(self, pad: int = 0) -> Sentence:
        ...
        extra = Sentence()
        if pad > 0:
            extra.addf(color, (8 - len(text)) * " ")

        if self.is_true():
            value = Sentence().addf(color + "/", text).concat(extra).addf("", f"[{char}]")
        else:
            value = Sentence().addf("", f"[{char}]").concat(extra).addf(color + "/", text)

        return value
    

class Flags:
    count = Flag().name("Count").char("C").values([0, 1]).text("Mostra a contagem de tarefas").left_bar()
    down = Flag().name("Down").char("D").values([0, 1]).text("Mostra o caminho para a tarefa baixadas").left_bar()
    minimum = Flag().name("Minimum").char("M").values([0, 1]).text("Mostra a nota mínima para a tarefa").left_bar()
    opt = Flag().name("Opt").char("O").values([0, 1]).text("Mostra tarefas opcionais").left_bar()
    xp = Flag().name("Xp").char("X").values([0, 1]).text("Mostra a xp obtida").left_bar()
    percent = Flag().name("Percent").char("P").values([0, 1]).text("Mostra a porcentagem de tarefas").left_bar()
    admin = Flag().name("Admin").char("A").values([0, 1]).text("Mostra todas as missões e grupos").left_bar()

    dots = Flag().name("Dots").char(".").values([1, 0]).text("Mostra o preenchimento com pontos").left_bar()
    group_prog = Flag().name("Group").char("G").values([1, 0]).text("Mostra a barra de progresso dos grupos").left_bar()
    quest_prog = Flag().name("Quest").char("Q").values([1, 0]).text("Mostra a barra de progresso das missões").left_bar()

    help_bar = Flag().name("HelpBar").char("H").values([1, 0]).text("Mostra a barra de ajuda")
    skills_bar = Flag().name("SkillsBar").char("S").values([1, 0]).text("Mostra a barra de skills")
    flags_bar = Flag().name("FlagsBar").char("F").values([1, 0]).text("Mostra a barra de flags")
...
class Play:
    ...
    def show_flags_bar(self, frame: Frame):
        ...
        for flag in self.flagsman.left:
            frame.print(1, flag.get_toggle_sentence(8))
