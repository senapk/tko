...
from tko.play.tester_executor import TesterExecutor
from tko.play.tester_navigator import TesterNavigator
from tko.play.tester_palette import TesterPalette
from tko.play.tester_renderer import TesterRenderer
from tko.play.tester_state import TesterState, SeqMode
from tko.play.tester_top_bar import TesterTopBar
...
class Tester:
    ...
    def __init__(self, settings: Settings, rep: Repository | None, wdir: Wdir, task: Task) -> None:
        ...
        if rep:
            rep.logger.store(
                LogItemMove().set_mode(LogItemMove.Mode.PICK).set_key(task.get_full_key())
            )
            ...
            if free_run_fn is None:
                if self.rep:
                    self.rep.logger.store(
                        ...
                        .set_key(self.task.get_full_key())
                    )
                break
            else:
                while True:
