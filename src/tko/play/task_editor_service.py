        ...
        try:
            ...
            if isinstance(obj, Task):
                ...
                if not track_folder.exists():
                    track_folder = self.repo.paths.get_legacy_track_task_folder(task.basic.full_key)
                if not track_folder.exists():
                    return lambda: None
                files = [file for file in track_folder.rglob("*") if file.is_file() and file.suffix in (".json", ".jsonl")]
