...
class CollectActions:
    ...
    def get_task_history(repo: Repository) -> list[TaskCollected]:
        ...
        """
        remote_paths: dict[str, str] = {remote.name: remote.path_or_url for remote in repo.remotes.values()}
    ...
    def get_resume(repo: Repository) -> dict[str, TaskCollected]:
        ...
        for key, log_sort in tasks.items():
            resume = TaskCollected().setup(log_sort, repo.game.get_task(key), {remote.name: remote.path_or_url for remote in repo.remotes.values()})
    ...
    def load_game_as_quest_list(rep: Repository) -> list[QuestGameData]:
        ...
        for quest in game.quests.values():
            ...
            for task in quest.get_tasks():
                output_quest.tasks.append(TaskGameData(key=task.basic.full_key, value=task.game.gain, is_leet=task.config.is_eval_test))
        return output
