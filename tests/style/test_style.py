...
class Test:    
    def test_convert(self):
        res = Border(False).build_bar("abcdefghij", 0.5, 10, "t", "f", round=False)
        val, fmt = res.resume_val_fmt()
        assert val == "abcdefghij"
        assert fmt == "tttttfffff"
    
    def test_convert2(self):
        res = Border(False).build_bar("abcdefghij", 1, 10, "t", "f", round=False)
        val, fmt = res.resume_val_fmt()
        assert val == "abcdefghij"
        assert fmt == "tttttttttt"
    
    def test_convert3(self):
        res = Border(False).build_bar("abcdefghij", 0, 10, "t", "f", round=False)
        val, fmt = res.resume_val_fmt()
        assert val == "abcdefghij"
        assert fmt == "ffffffffff"

    def test_convert4(self):
        res = Border(False).build_bar("abcdefghij", 0.5, 12, "t", "f", round=False)
        val, fmt = res.resume_val_fmt()
        assert val == " abcdefghij "
        assert fmt == "ttttttffffff"

    def test_convert5(self):
        res = Border(False).build_bar("abcdefghij", 0.5, 8, "t", "f", round=False)
        val, fmt = res.resume_val_fmt()
        assert val == "abcdefgh"
        assert fmt == "ttttffff"
