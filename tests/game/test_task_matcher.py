...
class TestTaskMatcher:
    def test_match_full_pattern_extracts_groups(self):
        ...
        line = "- [ ]   @chave :test:diff:type=diff [Minha tarefa](path/to/task.md)  #obs"
        found = matcher.match_pattern(line)

        assert found is True
        assert matcher.raw_pre == "   @chave :test:diff:type=diff "
    ...
    def test_match_full_pattern_filters_backticks_and_html_comment_tags(self):
        ...
        line = "- [ ] `@key type=wiki` <!--main:10--> [Titulo](task.md)"
        found = matcher.match_pattern(line)
    ...
    def test_match_full_pattern_accepts_checked_and_unchecked(self):
        ...
        found_checked = matcher.match_pattern("- [x] `type=wiki` [Tarefa](task.md)")
        found_unchecked = matcher.match_pattern("- [ ] `type=wiki` [Tarefa](task.md)")
    ...
    def test_match_full_pattern_invalid_line_does_not_overwrite_previous_state(self):
        ...
        assert matcher.match_pattern("- [ ] @k :diff:type=diff [  Titulo ](task.md) after") is True

        found = matcher.match_pattern("linha sem formato")

        assert found is False
        assert matcher.raw_pre == " @k :diff:type=diff "
    ...
    def test_match_pattern_with_gain_hard_size_eval_type(self):
        matcher = TaskMatcher()
        line = "- [ ] `@foo gain=3 hard=2 size=4 type=self` [Titulo](task/README.md)"
        ...
        assert matcher.resource_type == TaskType.SELF
        assert matcher.get_filled_fields() == ["@foo", "type=self", "gain=3", "hard=2", "size=4"]

    def test_match_pattern_with_legacy_xp_tier(self):
        matcher = TaskMatcher()
        line = "- [ ] `@bar xp=5 tier=3 type=diff` [Titulo](task/README.md)"
        assert matcher.match_pattern(line) is True
        ...
        assert matcher.is_make is True
        assert matcher.resource_type == TaskType.DIFF
        assert matcher.get_filled_fields() == ["@bar", "type=diff", "gain=5", "hard=3", "size=1"]

    def test_read_task_defaults_and_filled_fields(self):
        matcher = TaskMatcher()
        line = "- [ ] `@read_task type=wiki` [Material](wiki/git/README.md)"
        assert matcher.match_pattern(line) is True
        assert matcher.key == "read_task"
        assert matcher.is_read is True
        assert matcher.get_filled_fields() == ["@read_task", "type=wiki", "gain=1", "hard=1", "size=1"]

    def test_read_task_keeps_hard_and_size_but_omits_eval(self):
        matcher = TaskMatcher()
        line = "- [ ] `@read_task gain=5 hard=3 size=2 type=wiki` [Material](wiki/git/README.md)"

        assert matcher.match_pattern(line) is True
        assert matcher.get_filled_fields() == ["@read_task", "type=wiki", "gain=5", "hard=3", "size=2"]
    ...
    def test_read_task_eval_test_is_normalized_to_self(self):
        ...
        assert matcher.match_pattern(line) is True
        assert matcher.resource_type == TaskType.WIKI
        assert matcher.get_filled_fields() == ["@read_task", "type=wiki", "gain=1", "hard=1", "size=1"]
