...
class Test:
    ...
    def test_edit_task_with_github_blob_url_is_parsed_and_redirected(self):
        tp = TaskParser(
        ...
        task = tp.parse_line(
            "- [ ] [@label complemente](https://github.com/user/repo/blob/main/folder/file.md)",
            7,
        )
    ...
    def test_edit_task_with_github_tree_url_is_parsed_and_redirected(self):
        tp = TaskParser(
            index_path=Path("/source/arquivo.md"),
            remote_dir_root=Path("/source"),
            remote_name="poo",
        ...
        task = tp.parse_line(
            "- [ ] [@label complemente](https://github.com/user/repo/tree/main/folder/sub)",
            8,
        )
    ...
    def test_edit_task_with_external_non_github_url_becomes_view(self):
        tp = TaskParser(
            index_path=Path("/source/arquivo.md"),
            remote_dir_root=Path("/source"),
            remote_name="poo",
        ...
        task = tp.parse_line(
            "- [ ] [@label complemente](https://example.com/material)",
            9,
        )

        assert task is not None
        assert task.resource.resource_type == ResourceType.VIEW
    ...
    def test_view_task_external_url_sets_default_free_and_self(self):
        tp = TaskParser(index_path=Path("/source/arquivo.md"), remote_dir_root=Path("/source"), remote_name="poo")
        task = tp.parse_line("- [ ] [@+ref material](https://example.com/material)", 3)
    ...
    def test_decode_task_types_sets_expected_values(self):
        tp = TaskParser(index_path=Path("/source/arquivo.md"), remote_dir_root=Path("/source"), remote_name="poo")
        tp.decode_task_types("15:test:perk:zero")

        assert tp.task.game.xp == 15
        assert tp.task.config.test == TaskTest.TEST
        assert tp.task.config.path == TaskMain.PERK
        assert tp.task.config.loss == TaskLoss.ZERO

    def test_decode_task_types_unknown_tag_prints_warning(self, caplog: LogCaptureFixture):
        tp = TaskParser(index_path=Path("/source/arquivo.md"), remote_dir_root=Path("/source"), remote_name="poo")
        tp.task.resource.line_number = 11
        with caplog.at_level("WARNING"):
            tp.decode_task_types("desconhecido")
        assert "Tipo de tarefa desconhecido" in caplog.text
    ...
    def test_decode_task_types_covers_self_main_side_free_and_part(self):
        tp = TaskParser(index_path=Path("/source/arquivo.md"), remote_dir_root=Path("/source"), remote_name="poo")
        tp.decode_task_types("self:main:side:free:part")

        # Last tag wins for path/loss in this sequence.
        assert tp.task.config.test == TaskTest.SELF
        assert tp.task.config.path == TaskMain.SIDE
        assert tp.task.config.loss == TaskLoss.PART
