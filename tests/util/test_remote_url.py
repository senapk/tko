...
def test_url_without_https_raises_value_error() -> None:
    with pytest.raises(ValueError):
        GitHubUrlDownloader("http://github.com/user/repo/blob/main/folder/file.md")


def test_url_without_scheme_raises_value_error() -> None:
    with pytest.raises(ValueError):
        GitHubUrlDownloader("github.com/user/repo/blob/main/folder/file.md")


def test_gist_url_sets_raw_link() -> None:
    url: str = "https://gist.githubusercontent.com/user/abc123/raw/abc/file.md"
    r: GitHubUrlDownloader = GitHubUrlDownloader(url)
...
def test_invalid_github_url_raises_value_error() -> None:
    with pytest.raises(ValueError):
        GitHubUrlDownloader("https://github.com/user/repo/invalid/path/file.md")
